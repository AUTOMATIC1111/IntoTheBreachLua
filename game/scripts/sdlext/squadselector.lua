...
local function loadSquadSelection()
	local map = {}
...
function modApi.selectSquads()
	...
	sdlext.uiEventLoop(function(ui,quit)
		...
		for i=1,#modApi.mod_squads do
			...
			local checkbox = UiCheckbox():pos(0.5*col,0):setypx(80*row):heightpx(41):width(0.48):decorate(
				{ DecoButton(), DecoCheckbox(), DecoSurfaceOutlined(surface), DecoText(modApi.squad_text[(i-1)*2+1]) }
			)
